...
class Drawer:
    ...
    def cute_draw(self, curr, parents=set(), show=False, onAPRays=True):
        k = 50
        r = 0.25 * k
        rr = 0.05 * k
        ...
        if curr in parents:
            ...
            static_draw.line(((curr.j * k, curr.i * k), (curr.parent.j * k, curr.parent.i * k)), fill=(189, 42, 132, 100), width = 5)
...
def draw_static(grid_map, start = None, goal = None, path = None):
    k = 50
    r = 0.25 * k
    rr = 0.05 * k
    ...
    if path is not None:
        for step in path:
            draw.line(((step.j * k, step.i * k), (step.parent.j * k, step.parent.i * k)), fill=(189, 42, 132))
...
def draw_png(grid_map, start = None, goal = None):
    result = theta_ap(grid_map, start[0], start[1], goal[0], goal[1], euclidian_distance, SearchTreePQS)
    path = make_path(result[1])
    draw_static(grid_map, Point(start[0], start[1]), Point(goal[0], goal[1]), path[0])
